...
async def seed_alert(session_factory, **overrides):
    """Create a full chain of WatchQuery -> RetailerUrl -> ScrapeJob -> ScrapeResult -> Alert."""
    async with session_factory() as session:
        wq = WatchQuery(
...
async def test_list_alerts_pagination(client_with_db):
    ...
    # Get page with limit=1, offset=1 (should skip newest, return middle)
